...
echo "# Configure Database for ${database_type} ..."

if [ ${database_type} == 'mysql' ]; then
echo | mysql -uroot ${dcmgr_dbname} && yes | db_drop ${dcmgr_dbname} >/dev/null 2>&1
echo | mysql -uroot ${webui_dbname} && yes | db_drop ${webui_dbname} >/dev/null 2>&1
cat <<EOS | mysql -uroot
...
echo | psql -U postgres ${dcmgr_dbname} && yes | db_drop ${dcmgr_dbname} >/dev/null 2>&1
echo | psql -U postgres ${webui_dbname} && yes | db_drop ${webui_dbname} >/dev/null 2>&1
cat <<EOS | ${db_admin_login} postgres
create database ${dcmgr_dbname} encoding 'utf8';
create database ${webui_dbname} encoding 'utf8';
create user ${webui_dbname} with password '${webui_dbpass:-passwd}';
grant all privileges on database ${webui_dbname} to ${webui_dbname};
EOS
fi
